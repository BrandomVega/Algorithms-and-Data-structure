...
void extraePorN(Lista* l, int* e, int n,Dato* datoExtraido){
    ...
    if((*l)->cabecera==NULL){
        ...
        struct Nodo* nodoAux = (struct Nodo*)malloc(sizeof(struct Nodo)); //Para el dato extraido
        ...
        nodoAux = anterior->siguiente;
        *datoExtraido = nodoAux->dato;

        printf("El nodo a extraer tiene el valor de: %d \n", datoExtraido->valor);

        //Encontramos el siguiente al que se extraera
        struct Nodo* temp = (struct Nodo*)malloc(sizeof(struct Nodo));
        temp = anterior->siguiente->siguiente;

        printf("El nodo a enlazar tiene el valor de: %d \n", temp->dato.valor);

        anterior->siguiente = temp;

        printf("El nodo enlazado tiene el valor de: %d \n", anterior->siguiente->dato.valor);


        printf("El nodo salvado tiene el valor de: %d \n", nodoAux->siguiente->dato.valor);

        //Separando el nodoaux del dato siguiente;
